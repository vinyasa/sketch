...
export async function renderAssemblyThumbnail(boards) {
    ...
    boards.forEach(b => {
        if (b.visible === false) return;
        minX = Math.min(minX, b.position[0] - b.size[0] / 2);
        minY = Math.min(minY, b.position[1] - b.size[1] / 2);
        minZ = Math.min(minZ, b.position[2] - b.size[2] / 2);
        maxX = Math.max(maxX, b.position[0] + b.size[0] / 2);
        maxY = Math.max(maxY, b.position[1] + b.size[1] / 2);
        maxZ = Math.max(maxZ, b.position[2] + b.size[2] / 2);
    });
