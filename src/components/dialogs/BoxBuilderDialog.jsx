...
const BoxBuilderDialog = () => {
    const { boxDialog: dialog, setBoxDialog: setDialog, buildBox, units } = useStore();
    if (!dialog) return null;

    const isMetric = units === 'metric';

    const W_in = parseNum(dialog.width, 18);
    const H_in = parseNum(dialog.height, 12);
    const D_in = parseNum(dialog.depth, 12);
    const tTB_in = parseNum(dialog.thicknessTB, 0.5);
    const tSide_in = parseNum(dialog.thicknessSide, 0.5);
    const tFront_in = parseNum(dialog.thicknessFront, 0.5);
    const tBack_in = parseNum(dialog.thicknessBack, 0.5);
                ...
                <div className="inspector-card" style={{ margin: 0 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                        ...
                        <div>
                            ...
                            <NumericInput step={isMetric ? "1" : "0.0625"} min="0.125" value={tFront}
                                onChange={val => setDialog(p => ({ ...p, thicknessFront: isMetric ? val / 25.4 : val }))}
                                style={inputStyle} />
                        </div>
                        <div>
                ...
                <div className="inspector-card" style={{ margin: 0, background: valid ? 'rgba(60,200,90,0.06)' : 'rgba(255,59,48,0.06)', border: valid ? '1px solid rgba(60,200,90,0.2)' : '1px solid rgba(255,59,48,0.3)' }}>
                    ...
                    {valid ? (
                        <div style={{ fontSize: '0.78rem', color: 'var(--text-main)', lineHeight: 1.6 }}>
                            <div><strong>Top / Bottom:</strong> {fmt(topBottom.x)}{unitLabel} × {fmt(topBottom.y)}{unitLabel} × {fmt(topBottom.z)}{unitLabel} <span style={{ color: 'var(--text-muted)' }}>(flush, no overlap)</span></div>
                            <div><strong>Front / Back:</strong> {fmt(frontBack.x)}{unitLabel} × {fmt(frontBack.y)}{unitLabel} × {fmt(frontBack.z)}{unitLabel} <span style={{ color: 'var(--text-muted)' }}>(between top/bottom)</span></div>
                            <div><strong>Left / Right:</strong> {fmt(sides.x)}{unitLabel} × {fmt(sides.y)}{unitLabel} × {fmt(sides.z)}{unitLabel} <span style={{ color: 'var(--text-muted)' }}>(between front/back)</span></div>
